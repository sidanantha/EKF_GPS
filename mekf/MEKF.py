...
class MEKF:
    ...
    def __init__(self, initial_est, estimate_covariance, 
        ...
        # Init the sensor bias, assume zero to start:
        self.gyro_bias = np.array([0.0, 0.0, 0.0])
        self.accelerometer_bias = np.array([0.0, 0.0, 0.0])
        # These biases get propated in the filter, and updated in the update step.

        self.F = np.zeros(shape=(15, 15), dtype=float)
        self.F[0:3, 9:12] = -np.identity(3)
        self.F[6:9, 3:6] =  np.identity(3)
    ...
    def process_covariance(self, time_delta):
        # Generat the large process noise covariance matrix:
        # grounded in knowledge of uncertainty in the gyro and accelerometer dynamics propagation
    ...
    def update(self, gyro_meas, acc_meas, time_delta):
        ...
        # We have an estimate for the bias, so we can subtract it from the measurements:
        gyro_meas = gyro_meas - self.gyro_bias
        acc_meas = acc_meas - self.accelerometer_bias
        # The measurement may not be perfect, we are just subtracting out our ESTIMATED bias

        # Quaternion state prediction:
        ...
        self.F[3:6, 0:3] = -quatToMatrix(self.estimate).dot(skewSymmetric(acc_meas))
        self.F[3:6, 12:15] = -quatToMatrix(self.estimate)
        Phi = np.identity(15, dtype=float) + self.F*time_delta

        #Update with a priori covariance
        self.estimate_covariance = np.dot(np.dot(Phi, self.estimate_covariance), Phi.transpose()) + self.process_covariance(time_delta)
        ...
        # Renormalize quaternion with the error term:
